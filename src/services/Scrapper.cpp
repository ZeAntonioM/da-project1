...
void Scrapper::scrapeLines(Graph &graph, string lines_file)
{
    ...
    while (getline(file, line))
    {
        ...
        if (line_service == "STANDARD")
            line_service_ = STANDARD;
        else
            line_service_ = ALFA;
        auto v1 = graph.findStation(station_A);
        auto v2 = graph.findStation(station_B);
        if (v1 == nullptr)
            cout << station_A << " Not found" << endl;
        if (v2 == nullptr)
            cout << station_B << " Not found" << endl;
        graph.addBidirectionalLine(v1, v2, stoi(capacity), line_service_);
        lines.push_back(
            Line(graph.findStation(station_A), graph.findStation(station_B), stoi(capacity), line_service_));
    }
}
