...
pair<Stations, Lines> Reports::getToDisable() const
{
    ...
    while (true)
    {
        ...
        if (station_name == "q" or station_name == "Q")
            break;
        auto station = graph->findStation(station_name);
        if (station == nullptr)
            cout << station_name << " is not a station!Insert a station name or q to finish!\n";
        else
    ...
    while (true)
    {
        ...
        if (station_name_1 == "q" or station_name_1 == "Q")
            break;
        cout << "Insert the second station name: ";
        getline(cin, station_name_2);
        if (station_name_2 == "q" or station_name_2 == "Q")
            break;
        auto station = graph->findStation(station_name_1);
        if (station == nullptr)
            cout << station_name_1 << " is not a station!Insert a station name or q to finish!\n";
        auto station2 = graph->findStation(station_name_2);
        if (station2 == nullptr)
            cout << station_name_2 << " is not a station!Insert a station name or q to finish!\n";
        bool found = false;
        ...
        if (!found)
            cout << "There is no line that connects " << station_name_1 << " with " << station_name_2 << endl;
    }
