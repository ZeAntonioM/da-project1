...
void GeneralReport::draw(DestinationsReport::DestResults destReportResults, DistrictsReport::DistResults distResults, ComponentsReport::CompResults compResults, ReachableReport::ReachResults reachResults) const
{
    int size = 83;
    system("clear");
...
void GeneralReport::doReport(Stations stationsToDisable, Lines linesToDisable, int &percentage)
{
    ...
    while (destPerc < 100)
        percentage = destPerc / 4;
    DestinationsReport::DestResults destReportResults = f.get();
    ...
    while (disPerc < 100)
        percentage = (destPerc + disPerc) / 4;
    DistrictsReport::DistResults distReportResults = f2.get();
    ...
    while (compPerc < 100)
        percentage = (destPerc + disPerc + compPerc) / 4;
    ComponentsReport::CompResults compReportResults = f3.get();
    ...
    while (reachPerc < 100)
        percentage = (destPerc + disPerc + compPerc + reachPerc) / 4;
    ReachableReport::ReachResults reachableReportResults = f4.get();
    percentage = 100;
    while (percentage != -1)
        ;
...
void GeneralReport::execute()
{
    ...
    int percentage = 0;
    auto func = [&](Stations stations, Lines lines, int *percentage)
    { doReport(stations, lines, *percentage); };
