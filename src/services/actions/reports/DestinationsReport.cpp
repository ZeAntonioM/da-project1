...
void DestinationsReport::draw(vector<pair<Station *, pair<int, int>>> results) const
{
    ...
    for (auto p : results)
    {
        ...
        cout << name;
        for (int i = 0; i < 37 - name.length() + specialChars(name); i++)
            cout << " ";
        cout << "│ " << p.second.first;
        for (int i = 0; i < 15 - to_string(p.second.first).length(); i++)
            cout << " ";
        cout << "│ " << p.second.second;
        for (int i = 0; i < 14 - to_string(p.second.second).length(); i++)
            cout << " ";
        cout << "\033[0m│" << endl;
    }
    cout << drawFooter(71);
...
DestinationsReport::doReport(vector<Station *> stations, vector<Line *> lines, int &percentage)
{
    ...
    {
        if (before[i].first == -1 or after[i].first == -1)
            continue;
...
void DestinationsReport::execute()
{
    ...
    int percentage = 0;
    auto report = [&](pair<Stations, Lines> data, int *percentage)
    {
        return this->doReport(data.first, data.second, *percentage);
    };
