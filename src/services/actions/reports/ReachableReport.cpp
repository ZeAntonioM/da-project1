...
void ReachableReport::draw(ReachResults stations)
{
...
ReachableReport::ReachResults ReachableReport::doReport(Stations stationsToDisable, Lines linesToDisable, int &percentage)
{
    ...
    for (auto station : graph->getStationVector())
    {
        ...
        if (!station->isDisabled())
            res.second.first = graph->bfs(station);
    ...
    {
        graph->reset();
        if (!station.first->isDisabled())
            station.second.second = graph->bfs(station.first);
    }
    ...
    {
        if (stations[i].second.second != -1 and stations[i].second.first != -1)
            pq.push(make_pair(stations[i].second.first - stations[i].second.second, i));
    }
